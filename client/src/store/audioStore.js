...
/**
 * Audio store — manages MULTIPLE background audio tracks per design.
 * Each track has its own HTMLAudioElement, trim window, volume, loop,
 * and a timelineOffset (seconds from start of video timeline where this
 * track begins playing).
 *
 * Key behaviours (Canva-style):
 *  - Individual play buttons in AudioPlayerBar do NOT auto-play other tracks.
 *  - When the video timeline plays, each audio track fires at its own
 *    timelineOffset and stops at timelineOffset + clipDuration.
 *  - Tracks can be repositioned left/right on the waveform strip to set
 *    their timelineOffset.
 */
...
export const useAudioStore = create((set, get) => ({
  ...
  addTrack(trackInfo) {
      ...
      set(s => {
        ...
        if (end && audio.currentTime >= end) {
          if (t.loop) {
            ...
            updated[idx] = { ...t, isPlaying: false, currentTime: t.trimStart || 0 }
    ...
    audio.onplay  = () => set(s => ({ tracks: s.tracks.map(t => t.track.id === trackInfo.id ? { ...t, isPlaying: true  } : t) }))
    audio.onpause = () => set(s => ({ tracks: s.tracks.map(t => t.track.id === trackInfo.id ? { ...t, isPlaying: false } : t) }))
    audio.onended = () => set(s => ({ tracks: s.tracks.map(t => t.track.id === trackInfo.id ? { ...t, isPlaying: false } : t) }))
  ...
  play(trackId) {
    ...
    if (!audio.src) { audio.src = t.track.src; audio.load() }
    if (audio.currentTime < (t.trimStart || 0) ||
  ...
  seek(trackId, time) {
    ...
    set(s => ({ tracks: s.tracks.map(t => t.track.id === trackId ? { ...t, currentTime: time } : t) }))
  },
  ...
  seekAllToPlayhead(playheadSec) {
    get().tracks.forEach(t => {
      ...
      if (playheadSec >= offset && playheadSec < offset + clipDur) {
        audio.currentTime = Math.max(t.trimStart || 0, Math.min(posInTrack, t.trimEnd ?? audio.duration ?? Infinity))
      }
    })
  },
  ...
  setVolume(trackId, v) {
    ...
    set(s => ({ tracks: s.tracks.map(t => t.track.id === trackId ? { ...t, volume: v } : t) }))
  },

  setLoop(trackId, v) {
    set(s => ({ tracks: s.tracks.map(t => t.track.id === trackId ? { ...t, loop: v } : t) }))
  },

  setTrim(trackId, trimStart, trimEnd) {
    const audio = getOrCreateEl(trackId)
    if (audio) {
      if (audio.currentTime < trimStart || (trimEnd && audio.currentTime > trimEnd)) {
        audio.currentTime = trimStart
      }
    }
    set(s => ({ tracks: s.tracks.map(t => t.track.id === trackId ? { ...t, trimStart, trimEnd } : t) }))
  },
  ...
  setTimelineOffset(trackId, offsetSec) {
    set(s => ({
      tracks: s.tracks.map(t =>
        t.track.id === trackId ? { ...t, timelineOffset: Math.max(0, offsetSec) } : t
      ),
    }))
  },
  ...
  serialise() {
    return get().tracks.map(({ track, trimStart, trimEnd, timelineOffset, volume, loop }) => ({
      track, trimStart, trimEnd, timelineOffset: timelineOffset || 0, volume, loop,
    }))
  },
  ...
  restore(saved) {
    if (!Array.isArray(saved)) {
      if (saved?.track?.src) {
        const { track, trimStart = 0, trimEnd = null, timelineOffset = 0, volume = 0.8, loop = false } = saved
        get().addTrack(track)
        setTimeout(() => {
    ...
    saved.forEach(({ track, trimStart = 0, trimEnd = null, timelineOffset = 0, volume = 0.8, loop = false }) => {
      if (!track?.src) return
      get().addTrack(track)
      setTimeout(() => {
        get().setTrim(track.id, trimStart, trimEnd)
  ...
  // Legacy compat
  get track() { return get().tracks[0]?.track ?? null },
  setTrack(trackInfo) { get().addTrack(trackInfo) },
  stop() { get().stopAll() },
}))
