...
function ShapesDialog({ onClose, currentTool, onSelect }) {
  ...
  return (
    <div
      ref={ref}
      className="absolute top-14 left-1/2 -translate-x-1/2 z-50 bg-white rounded-2xl shadow-2xl border border-gray-100 p-4 animate-scale-in"
      style={{ width: 340 }}
    >
...
function ExportDialog({ stageRef, onClose }) {
    ...
    for (const shape of videoShapes) {
      ...
      if (v && v.readyState >= 2) {
        ...
        ctx.drawImage(
          ...
          shape.x * pixelRatio,
          shape.y * pixelRatio,
          shape.width * pixelRatio,
          shape.height * pixelRatio,
        );
        ctx.restore();
      }
    }
  ...
  const handleExport = async () => {
    try {
      ...
      const stage = stageRef.current;
      if (!stage) {
        toast.error("Stage not ready. Please wait and try again.");
        return;
      }
      ...
      if (format === "webm") {
        ...
        let staticBg = null;
        try {
          const konvaStage2 = stageRef.current?.getStage
            ? stageRef.current.getStage()
            : stageRef.current;
          const prevX = konvaStage2.x(),
            prevY = konvaStage2.y(),
            prevSc = konvaStage2.scaleX();
          konvaStage2.position({ x: 0, y: 0 });
          konvaStage2.scale({ x: 1, y: 1 });
          const dataUrl = konvaStage2.toDataURL({
            x: 0,
            y: 0,
            width: cw2,
            height: ch2,
          ...
          konvaStage2.position({ x: prevX, y: prevY });
          konvaStage2.scale({ x: prevSc, y: prevSc });
          ...
          try {
            ...
            for (const trackState of activeTracks) {
              ...
              try {
                ...
                const offsetMs = Math.round(
                  (trackState.timelineOffset ?? 0) * 1000,
                );
                if (offsetMs <= 0) {
                  await el.play().catch(() => {});
                } else {
            ...
            if (exportAudioEls.length > 0) {
              audioDest.stream
                ...
                .forEach((t) => combinedStream.addTrack(t));
              setExportProgress(
                `Recording ${exportSec}s with ${exportAudioEls.length} audio track${exportAudioEls.length > 1 ? "s" : ""}…`,
              );
            } else {
        ...
        recorder.onstop = () => {
          ...
          if (exportAudioEls.length > 0) {
            try {
              exportAudioEls.forEach((el) => el.pause());
            } catch {}
          }
          if (audioCtx) {
            try {
          ...
          toast.success(
            `Video exported! (${exportSec}s${activeTracks.length > 0 ? ` · ${activeTracks.length} audio track${activeTracks.length > 1 ? "s" : ""}` : ""})`,
          );
          onClose();
        };
        ...
        const interval = setInterval(() => {
          const remaining = Math.max(
            0,
            Math.ceil((exportDurationMs - (Date.now() - startTime)) / 1000),
          );
  ...
  const formats = [
    {
      id: "png",
      label: "PNG",
      desc: "Best quality, transparent background",
      icon: "🖼",
    },
    {
      id: "jpg",
      label: "JPG",
      desc: "Smaller file size, solid background",
      icon: "📷",
    },
    ...(isPresentation
      ? [
          {
            ...
            desc: "Export all pages as slides",
            icon: "📊",
          },
          {
            id: "pdf",
            label: "PDF",
            desc: "Export all pages as PDF",
            icon: "📄",
          },
        ]
      : [
  ...
  return (
    <div
      ref={ref}
      className="absolute top-14 right-4 z-50 bg-white rounded-2xl shadow-2xl border border-gray-100 p-5 animate-scale-in"
      style={{ width: 300 }}
    >
      <p className="text-sm font-semibold text-gray-800 mb-1">Export Design</p>
      <p className="text-xs text-gray-400 mb-4">
        Exports whiteboard canvas area only
      </p>
      ...
      <div className="space-y-2 mb-4">
        {formats.map((f) => (
          <label
            key={f.id}
            className={`flex items-start gap-3 p-2.5 rounded-xl cursor-pointer border-2 transition-all ${format === f.id ? "border-indigo-400 bg-indigo-50" : "border-transparent hover:bg-gray-50"}`}
          >
            <input
              type="radio"
              name="format"
...
export default function Toolbar({
  ...
  return (
    <div
      className="relative h-13 bg-white border-b border-gray-200 flex items-center px-4 gap-1.5 select-none shadow-sm"
      style={{ height: 52 }}
    >
      ...
      <button
        onClick={undo}
        title="Undo (Ctrl+Z)"
        className="w-9 h-9 rounded-xl flex items-center justify-center text-gray-500 hover:bg-gray-100 hover:text-gray-900 transition-all text-base"
      >
        ↩
      </button>
      <button
      ...
      {selectedId && (
        <>
          ...
          <button
            ...
            className="w-9 h-9 rounded-xl flex items-center justify-center text-red-400 hover:bg-red-50 hover:text-red-600 transition-all"
          >
            🗑
          </button>
        </>
      ...
      <div className="flex items-center gap-1 bg-gray-100 rounded-xl px-1 h-9">
        <button
          ...
          className="w-7 h-7 rounded-lg hover:bg-white hover:shadow-sm text-gray-600 font-bold transition-all"
        >
          −
        </button>
        <button
          onClick={fitCanvas}
          className="px-2 text-xs font-semibold text-gray-700 hover:text-indigo-600 min-w-[48px] text-center transition-colors"
        >
          {Math.round(zoom * 100)}%
        </button>
        <button
