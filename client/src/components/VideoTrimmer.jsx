...
/**
 * Canva-style video trimmer
 *
 * Fixes vs previous version:
 *  1. Filmstrip IS scrollable — outer div has overflow-x:auto, inner strip is
 *     position:relative with the real wide width. Drag handles read scroll
 *     offset from the correct scrollable ancestor.
 *  2. All drag coordinate math is done relative to the SCROLLABLE container
 *     (not the inner strip whose getBoundingClientRect moves with scroll).
 *  3. onApply writes trimStart/trimEnd with full precision.
 */
export default function VideoTrimmer({
  ...
  const scrollRef    = useRef()   // ← the overflow-x:auto container
  const [duration, setDuration]       = useState(propDuration || 0)
  const [currentTime, setCurrentTime] = useState(trimStart || 0)
  const [isPlaying, setIsPlaying]     = useState(false)
  const [start, setStart] = useState(trimStart || 0)
  // end = trimEnd if set, else propDuration if known, else 0 (updated after metadata loads)
  const [end, setEnd] = useState(
    trimEnd != null ? trimEnd : (propDuration || 0)
  )
  const [dragging,      setDragging]      = useState(null)
  const [frames,        setFrames]        = useState([])
  const [loadingFrames, setLoadingFrames] = useState(true)

  // Always-fresh refs so event handlers never stale-close
  ...
  // ── Layout constants ──────────────────────────────────────────────
  const FRAME_H    = 54
  const PX_PER_SEC = 80    // px per second in the strip
  const MIN_W      = 640   // minimum strip width even for short clips
  ...
  useEffect(() => {
    ...
    video.onloadedmetadata = async () => {
      ...
      setDuration(d)
      // Update end: use trimEnd if it was provided and valid, else full duration
      setEnd(prev => {
        if (trimEnd != null && trimEnd > 0 && trimEnd <= d) return trimEnd
        // Only reset to full duration if end is still at 0 or beyond real duration
  ...
  async function extractFrames(video, dur) {
    ...
    // ~1 frame every 3 seconds for long videos, min 8, max 120
    ...
    for (let i = 0; i < fc; i++) {
      ...
      await new Promise(resolve => {
        const timeout = setTimeout(() => {
          // Timeout seeking — draw what we have or a blank frame
          try { ctx.drawImage(video, 0, 0, 120, 68); result.push(cvs.toDataURL('image/jpeg', 0.45)) } catch {}
          resolve()
        }, 3000)  // 3s per frame max
    ...
    // Reset to trimStart without touching onseeked
    video.onseeked = null
    video.currentTime = startRef.current
  }
  ...
  // ── Drag — all coords relative to the scrollable container ───────
  /**
   * Returns X in strip-world coordinates (0 … stripWidth),
   * accounting for the scroll container's scroll position and
  ...
  useEffect(() => {
    ...
    function onMove(e) {
      ...
      if (dragging === 'start') {
        ...
        const ne = Math.max(startRef.current + 0.1, Math.min(t, durationRef.current))
        setEnd(ne)
      ...
      >
        ...
        >
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[10px] text-white/30">Drag the purple handles · scroll filmstrip to navigate</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        >
          <div className="flex items-center gap-2">
            ...
            <span className="font-mono text-xs text-violet-300 font-bold">{fmt(start)}</span>
            <span className="text-white/20 mx-1">→</span>
            <span className="text-[10px] text-white/40">End</span>
            <span className="font-mono text-xs text-violet-300 font-bold">{fmt(end)}</span>
          </div>
          <div className="flex items-center gap-3">
            <div className="flex items-center gap-1.5">
              <span className="text-[10px] text-white/30">Duration:</span>
              <span className="font-mono text-xs text-white/70 font-bold">{fmt(end - start)}</span>
            </div>
            <button
              onClick={() => { setStart(0); setEnd(duration); if (videoRef.current) videoRef.current.currentTime = 0 }}
          ...
          >
            ...
            >
              ...
              >
                {loadingFrames ? (
                  <div
                    ...
                    style={{ background: 'rgba(255,255,255,0.04)', minWidth: stripWidth }}
                  >
              ...
              >
                ...
                />
                <div
                  className="absolute -top-1.5 left-1/2 -translate-x-1/2 w-3.5 h-3.5 bg-white rounded-full shadow-xl"
                />
              </div>
        ...
        >
          <button
            onClick={() => { if (videoRef.current) videoRef.current.currentTime = start }}
          ...
          <button
            onClick={() => { if (videoRef.current) videoRef.current.currentTime = Math.max(start, end - 0.05) }}
            className="w-8 h-8 rounded-xl flex items-center justify-center text-white/50 hover:text-white hover:bg-white/10 transition-all"
