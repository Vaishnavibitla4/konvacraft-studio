  ...
  return (
    ...
    >
      <div className="flex items-start gap-3 px-4 py-3.5">
        ...
        <p className="text-white text-sm leading-relaxed font-medium pr-4">
          {message}
        </p>
      </div>
      <div
...
function ConfirmDialog({ data, onClose }) {
  ...
  return (
    ...
    >
      <div
        className="rounded-3xl border border-white/15 shadow-2xl p-6 max-w-sm w-full"
        style={{
          background:
            "linear-gradient(135deg,rgba(26,10,46,0.98),rgba(13,13,24,0.98))",
    ...
    >
      ...
      >
        ...
        <input
          ...
          onKeyDown={(e) => {
            if (e.key === "Enter") respond(value.trim() || null);
        ...
        <div className="flex gap-3">
          ...
          <button
            onClick={() => respond(value.trim() || null)}
            className="flex-1 py-2.5 rounded-xl text-sm font-bold text-white transition-all shadow-lg"
...
export function ToastProvider() {
  ...
  return (
    <>
      ...
      {/* Prompt */}
      {promptData && (
        <PromptDialog data={promptData} onClose={() => setPromptData(null)} />
      )}
    </>
  );
}
