...
export default function AssetPanel() {
  ...
  const handleUpload = useCallback(async (e) => {
    ...
    try {
      ...
      toast.success(
        `${isAudio ? "Audio" : isVideo ? "Video" : "Image"} uploaded!`,
      );
    } catch (err) {
  ...
  const images = assets.filter(
    (a) => a.resource_type !== "video" && !a.file_type?.startsWith("audio/"),
  );
  const videos = assets.filter((a) => a.resource_type === "video");
  const audios = assets.filter(
    (a) => a.resource_type === "audio" || a.file_type?.startsWith("audio/"),
  );
  ...
  return (
    <div className="flex h-full">
      {/* ── Sidebar icons ── */}
      <div
        className="w-14 flex flex-col items-center py-3 gap-1.5"
        ...
        {tab === "images" && (
          <>
              ...
              <div className="flex gap-2">
                ...
                {searchMode ? (
                  <button
                    ...
                    className="px-2.5 py-1.5 rounded-xl text-xs font-bold border border-violet-500/60 text-violet-300 bg-violet-950/40 hover:bg-violet-950/70 transition-all"
                  >
                    📁 Files
                  </button>
                ) : (
                  <button
                    ...
                    className="px-2.5 py-1.5 rounded-xl text-xs border border-white/10 text-white/40 hover:text-white/70 hover:bg-white/5 transition-all"
                  >
                    🔍 Search
                  </button>
                )}
            ...
            <div className="flex-1 overflow-y-auto">
              {searchMode ? (
                <div className="p-2">
                  <ImageSearch key="search" onSearch={() => {}} />
                </div>
              ) : (
                <>
                  <div className="p-2" style={{ borderBottom: border }}>
                    ...
                    >
                      <span>🔍</span>
                      <span>Search Unsplash / Pexels…</span>
                    </button>
                  </div>
        ...
        {tab === "shapes" && (
          <div className="flex-1 overflow-y-auto p-3">
            ...
            <p className="text-[10px] text-white/20 mb-3">
              Select tool, then click canvas to place
            </p>
            <div className="grid grid-cols-3 gap-2">
              {SHAPE_PRESETS.map((s) => (
                ...
                >
                  <span className="text-2xl text-white/70 group-hover:text-white transition-colors">
                    {s.icon}
                  </span>
                  ...
                  <div className="flex items-center justify-between gap-2">
                    <span
                      style={{
                        ...
                        fontSize: Math.min(p.fontSize * 0.45, 20),
                        fontWeight:
                          p.fontStyle === "bold" || p.fontStyle === "600"
                            ? 700
                            : 400,
                        color: p.color,
                      }}
...
function AudioPanel({
  ...
  function useTrack(a) {
    ...
    toast.success(
      "Audio added — timeline shown below canvas. Drag handles to trim.",
    );
  }
