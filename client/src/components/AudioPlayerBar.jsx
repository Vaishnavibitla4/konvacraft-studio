...
function TrackRow({ trackState, index, total, masterDuration = 30 }) {
  ...
  useEffect(() => {
    offsetRef.current = timelineOffset;
  }, [timelineOffset]);
  useEffect(() => {
    durRef.current = track.duration || 0;
  }, [track.duration]);
  useEffect(() => {
    masterDurRef.current = masterDuration || 30;
  }, [masterDuration]);
    ...
    function onMove(e) {
      ...
      if (dragging === "trim-start") {
        ...
        const newTrimStart = Math.max(
          0,
          Math.min(
            posInMaster - blockOff + (trimStartRef.current || 0),
            (trimEndRef.current ?? durRef.current) - 0.5,
          ),
        );
        // Keep block position fixed, only change trim window
        const clampedTs = Math.max(
          0,
          Math.min(newTrimStart, (trimEndRef.current ?? durRef.current) - 0.5),
        );
        ...
        const newTrimEnd =
          ...
          (posInMaster -
            blockOff -
            ((trimStartRef.current || 0) - (trimStartRef.current || 0)));
        // Simpler: trim-end moves right handle, shrinking/expanding clip window
        const clipRelative = posInMaster - blockOff;
        const newTe = Math.max(
          (trimStartRef.current || 0) + 0.5,
          Math.min((trimStartRef.current || 0) + clipRelative, durRef.current),
        );
        ...
        const clipD =
          (trimEndRef.current ?? durRef.current) - (trimStartRef.current || 0);
  ...
  const color2 =
    index % 2 === 0 ? "rgba(16,185,129,0.2)" : "rgba(124,58,237,0.2)";
      ...
      >
        ...
        <div className="flex items-center gap-2.5 px-3 pt-2 pb-1">
          ...
          <div className="min-w-0 shrink-0" style={{ maxWidth: 130 }}>
            ...
            <p
              className="text-[9px]"
              style={{ color: index % 2 === 0 ? "#6ee7b7" : "#c4b5fd" }}
            >
              {timelineOffset > 0 ? `@${fmt(timelineOffset)} · ` : ""}
              {fmt(trimStart)}–{fmt(clipEnd)}
            </p>
          </div>
          ...
          >
            ...
            {blockLeft > 0 && (
              <div
                className="absolute top-0 bottom-0 left-0 pointer-events-none rounded-l-lg"
                style={{
                  width: `${blockLeft * 100}%`,
            ...
            {blockRight < 1 && (
              <div
                className="absolute top-0 bottom-0 right-0 pointer-events-none rounded-r-lg"
                style={{
                  width: `${(1 - blockRight) * 100}%`,
                  background: "rgba(0,0,0,0.6)",
                  zIndex: 2,
                }}
              />
          ...
          {/* Elapsed */}
          <span className="text-[10px] font-mono text-white/35 shrink-0 w-8">
            {fmt(elapsed)}
          </span>
          ...
          <div className="flex items-center gap-1 shrink-0">
            ...
            <input
              ...
              style={{ width: 50, height: 4, accentColor: color1 }}
            />
          </div>

          {/* Loop */}
          <button
            onClick={() => setLoop(track.id, !loop)}
...
function SingleTrackBar({ trackState }) {
  const { track, isPlaying, volume, loop, currentTime, trimStart, trimEnd } =
    trackState;
  const { togglePlay, setVolume, setLoop, seek, removeTrack, setTrim } =
    useAudioStore();
  const [showTrimmer, setShowTrimmer] = useState(false);
    ...
    <>
      <div
        ...
        style={{
          height: 52,
          background:
            "linear-gradient(90deg,rgba(16,185,129,0.12),rgba(124,58,237,0.14),rgba(16,185,129,0.12))",
      ...
      >
            ...
            return (
              <div
                ...
                style={{
                  ...
                  height: `${height}%`,
                  background: isPlaying
                    ? `hsl(${160 + i * 2},70%,${50 + seed * 25}%)`
                    : "rgba(16,185,129,0.3)",
                  borderRadius: 2,
                  transformOrigin: "bottom",
                  animation: isPlaying
                    ? `barDance ${0.4 + seed * 0.6}s ease-in-out infinite`
                    : "none",
                  animationDelay: `${(i / BARS) * 0.4}s`,
                }}
              />
            );
          })}
        ...
        <div className="flex-1 flex items-center gap-2 min-w-0">
          ...
          <div
            ref={progressRef}
            className="flex-1 relative h-1.5 rounded-full cursor-pointer overflow-hidden"
        ...
        <button
          onClick={() => setLoop(track.id, !loop)}
          className={`text-xs px-2 py-1 rounded-lg border transition-all shrink-0 ${loop ? "border-emerald-500/60 text-emerald-300 bg-emerald-950/50" : "border-white/10 text-white/30 hover:text-white/60"}`}
        >
          🔁
        </button>
        <button
        ...
        <button
          onClick={() => removeTrack(track.id)}
          className="w-6 h-6 rounded-lg flex items-center justify-center text-white/30 hover:text-red-400 hover:bg-red-950/30 transition-all shrink-0 text-xs"
        >
          ✕
        </button>
      </div>
