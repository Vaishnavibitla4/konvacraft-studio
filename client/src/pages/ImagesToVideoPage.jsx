...
const FILTERS = [
  ...
  { id: 'cinematic', label: 'Cinematic', css: 'contrast(1.25) saturate(0.8) sepia(0.18)' },
  ...
  { id: 'vintage',   label: 'Vintage',   css: 'sepia(0.55) saturate(0.75) contrast(1.15)' },
  { id: 'golden',    label: 'Golden',    css: 'sepia(0.45) saturate(1.6) brightness(1.1)' },
  { id: 'dreamy',    label: 'Dreamy',    css: 'saturate(1.5) brightness(1.12)' },
  { id: 'matte',     label: 'Matte',     css: 'contrast(0.85) saturate(0.9) brightness(1.05)' },
]
...
const CLIP_COLORS = [
  ...
  '#0891b2','#db2777','#16a34a','#9333ea','#ea580c',
]
...
function renderTransitionFrame(ctx, img1, img2, progress, transition, w, h) {
  ...
  switch (transition) {
    case 'fade': case 'dissolve':
      if (img1) { ctx.globalAlpha = 1; ctx.drawImage(img1, 0, 0, w, h) }
      if (img2) { ctx.globalAlpha = p; ctx.drawImage(img2, 0, 0, w, h) }
      ctx.globalAlpha = 1; break
    case 'slideLeft':
      if (img1) ctx.drawImage(img1, -w * p, 0, w, h)
      if (img2) ctx.drawImage(img2,  w * (1 - p), 0, w, h); break
    case 'slideRight':
      if (img1) ctx.drawImage(img1,  w * p, 0, w, h)
      if (img2) ctx.drawImage(img2, -w * (1 - p), 0, w, h); break
    case 'slideUp':
      if (img1) ctx.drawImage(img1, 0, -h * p, w, h)
      if (img2) ctx.drawImage(img2, 0,  h * (1 - p), w, h); break
    case 'slideDown':
      if (img1) ctx.drawImage(img1, 0,  h * p, w, h)
      if (img2) ctx.drawImage(img2, 0, -h * (1 - p), w, h); break
    case 'zoomIn': {
      const s = 1 + p * 0.4
      if (img1) { ctx.save(); ctx.translate(w/2,h/2); ctx.scale(s,s); ctx.translate(-w/2,-h/2); ctx.drawImage(img1,0,0,w,h); ctx.restore() }
      if (img2) { ctx.globalAlpha = p; ctx.drawImage(img2,0,0,w,h); ctx.globalAlpha = 1 }
      break
    }
    case 'zoomOut': {
      const s2 = 1.4 - p * 0.4
      if (img1) ctx.drawImage(img1,0,0,w,h)
      if (img2) { ctx.save(); ctx.translate(w/2,h/2); ctx.scale(s2,s2); ctx.translate(-w/2,-h/2); ctx.globalAlpha=p; ctx.drawImage(img2,0,0,w,h); ctx.restore(); ctx.globalAlpha=1 }
      break
    }
    case 'wipe':
      if (img1) ctx.drawImage(img1,0,0,w,h)
      if (img2) { ctx.save(); ctx.beginPath(); ctx.rect(0,0,w*p,h); ctx.clip(); ctx.drawImage(img2,0,0,w,h); ctx.restore() }
      break
    case 'rotate':
      if (img1) { ctx.save(); ctx.translate(w/2,h/2); ctx.rotate(p*Math.PI*0.5); ctx.globalAlpha=1-p; ctx.drawImage(img1,-w/2,-h/2,w,h); ctx.restore() }
      if (img2) { ctx.save(); ctx.translate(w/2,h/2); ctx.rotate((p-1)*Math.PI*0.5); ctx.globalAlpha=p; ctx.drawImage(img2,-w/2,-h/2,w,h); ctx.restore(); ctx.globalAlpha=1 }
      break
    case 'kenBurns':
    default:
      if (img1) { ctx.globalAlpha=1; ctx.drawImage(img1,0,0,w,h) }
...
function drawAnimated(ctx, img, progress, animation, w, h, filterCss) {
  if (!img) { ctx.fillStyle='#000'; ctx.fillRect(0,0,w,h); return }
  ctx.save()
  switch (animation) {
    case 'kenBurns': {
      const sc=1.08+progress*0.07, ox=-(w*(sc-1))*0.4*(1-progress), oy=-(h*(sc-1))*0.3*(1-progress)
      if (filterCss) {
        const tmp=Object.assign(document.createElement('canvas'),{width:w,height:h})
        const tc=tmp.getContext('2d'); tc.filter=filterCss; tc.drawImage(img,ox,oy,w*sc,h*sc); ctx.drawImage(tmp,0,0)
      } else { ctx.drawImage(img,ox,oy,w*sc,h*sc) }
      break
    }
    case 'zoomIn':    { const s=1+progress*0.12; ctx.translate(w/2,h/2); ctx.scale(s,s); ctx.translate(-w/2,-h/2); ctx.drawImage(img,0,0,w,h); break }
    case 'zoomOut':   { const s2=1.12-progress*0.12; ctx.translate(w/2,h/2); ctx.scale(s2,s2); ctx.translate(-w/2,-h/2); ctx.drawImage(img,0,0,w,h); break }
    case 'slideLeft': { ctx.translate(-w*progress*0.08,0); ctx.drawImage(img,0,0,w,h); break }
    case 'slideRight':{ ctx.translate( w*progress*0.08,0); ctx.drawImage(img,0,0,w,h); break }
    case 'fadeIn':    { ctx.globalAlpha=progress; ctx.drawImage(img,0,0,w,h); break }
    case 'rotateIn':  { ctx.translate(w/2,h/2); ctx.rotate((1-progress)*Math.PI*0.08); ctx.globalAlpha=progress; ctx.drawImage(img,-w/2,-h/2,w,h); break }
    default: {
      if (filterCss) {
        const tmp2=Object.assign(document.createElement('canvas'),{width:w,height:h})
        const tc2=tmp2.getContext('2d'); tc2.filter=filterCss; tc2.drawImage(img,0,0,w,h); ctx.drawImage(tmp2,0,0)
...
function TransitionPill({ transition, onClick }) {
  ...
  return (
    <button onClick={onClick} title={`Transition: ${t.label} — click to change`}
      ...
      style={{
        background: transition === 'none' ? 'rgba(255,255,255,0.06)' : 'rgba(124,58,237,0.35)',
        borderColor: transition === 'none' ? 'rgba(255,255,255,0.1)' : 'rgba(124,58,237,0.6)',
      }}>
      {t.icon}
    </button>
  )
...
function TransitionPicker({ value, onSelect, onClose }) {
  return (
    <div className="absolute bottom-full mb-2 left-1/2 -translate-x-1/2 z-50 rounded-2xl border border-white/15 shadow-2xl p-3"
      style={{ background: 'rgba(18,14,36,0.98)', width: 260, backdropFilter: 'blur(16px)' }}>
      <div className="flex items-center justify-between mb-2">
        <p className="text-[10px] font-bold text-white/50 uppercase tracking-wider">Transition</p>
      ...
      <div className="grid grid-cols-4 gap-1.5">
        {TRANSITIONS.map(t => (
          <button key={t.id} onClick={() => { onSelect(t.id); onClose() }}
            ...
            <span className="text-[8px] text-white/50 leading-tight">{t.label}</span>
          </button>
        ))}
      </div>
    </div>
...
// ─────────────────────────────────────────────────────────────────────────────
// CANVA-STYLE TIMELINE  ← the main one below the canvas
// ─────────────────────────────────────────────────────────────────────────────
function VideoTimeline({ slides, selectedIdx, onSelectSlide, onUpdateSlide, onDeleteSlide,
                         audioStore, isPlaying, playheadSec, totalDuration, onTogglePlay, onSeek }) {
  const timelineRef = useRef()
  const [openTransIdx, setOpenTransIdx] = useState(null)
  const progress = totalDuration > 0 ? Math.min(1, playheadSec / totalDuration) : 0

  // Close transition popover on outside click
  useEffect(() => {
    if (openTransIdx === null) return
    const h = e => { if (!e.target.closest('[data-trans-picker]')) setOpenTransIdx(null) }
  ...
  function clickScrubber(e) {
    ...
    onSeek(Math.max(0, Math.min(1, (e.clientX - rect.left) / rect.width)) * totalDuration)
  }

  return (
    <div className="shrink-0 select-none" style={{ background: 'rgba(10,8,22,0.97)', borderTop: '1px solid rgba(124,58,237,0.25)' }}>

      {/* ── Transport bar ──────────────────────────────────────────────── */}
      <div className="flex items-center gap-3 px-4 pt-2.5 pb-2">
        {/* Play/Pause */}
        <button onClick={onTogglePlay} disabled={!slides.length}
          ...
          style={{ background: isPlaying ? 'rgba(236,72,153,0.75)' : 'linear-gradient(135deg,#7c3aed,#6d28d9)' }}>
          {isPlaying ? '⏸' : '▶'}
        </button>
        ...
        <div ref={timelineRef}
          ...
          <div className="absolute top-1/2 -translate-y-1/2 w-3 h-3 rounded-full border-2 border-white pointer-events-none"
            style={{ left: `calc(${progress * 100}% - 6px)`, background: '#ec4899' }} />
        </div>

        <span className="text-[9px] text-white/30 shrink-0">
          {slides.length} clip{slides.length !== 1 ? 's' : ''}{audioStore.tracks?.length > 0 ? ` · 🎵×${audioStore.tracks.length}` : ''}
        </span>
      </div>

      {/* ── Audio track lanes (multi-track) ───────────────────────────── */}
      {audioStore.tracks?.length > 0 && (
        <div className="flex flex-col gap-0.5 px-4 pb-1 border-t border-white/5"
          style={{ background: 'rgba(16,185,129,0.04)' }}>
          {audioStore.tracks.map((t, i) => {
            const trackDur  = (t.trimEnd ?? t.track?.duration ?? totalDuration) - (t.trimStart ?? 0)
            const offset    = t.timelineOffset ?? 0
            const leftPct   = totalDuration > 0 ? Math.min(100, (offset / totalDuration) * 100) : 0
            const widthPct  = totalDuration > 0 ? Math.min(100 - leftPct, (trackDur / totalDuration) * 100) : 100
            const trackColor = i % 2 === 0 ? 'rgba(16,185,129,' : 'rgba(124,58,237,'
            return (
              <div key={t.track.id} className="flex items-center gap-3 pt-1 pb-0.5">
                <span className="text-[9px] font-bold shrink-0 w-12"
                  style={{ color: i % 2 === 0 ? '#34d399' : '#a78bfa' }}>
                  AUDIO {i + 1}
                </span>
                <div className="flex-1 relative h-5 rounded-lg overflow-visible"
                  style={{ background: 'rgba(255,255,255,0.04)' }}>
                  <div className="absolute top-0 h-full rounded-lg flex items-center px-2 overflow-hidden"
                    style={{
                      left: `${leftPct}%`,
                  ...
                  <div className="absolute top-0 h-full w-px pointer-events-none z-10"
      ...
      <div className="px-4 pb-3 pt-1">
        {slides.length === 0 ? (
          <div className="h-14 rounded-lg border border-dashed border-white/10 flex items-center justify-center">
            <p className="text-[10px] text-white/20">Click images in the left panel to add slides</p>
          </div>
        ) : (
          <>
            <div className="relative h-14" data-timeline="1">
              ...
              <div className="relative w-full h-full flex items-center">
                {slides.map((slide, i) => (
                  <div key={slide.id} className="relative flex items-center h-full" style={{ flex: slide.duration || 4, minWidth: 50 }}>
                    ...
                    {i > 0 && (
                      <div className="relative shrink-0" data-trans-picker>
                        <TransitionPill
                          ...
                          onClick={e => { e.stopPropagation(); setOpenTransIdx(openTransIdx === i ? null : i) }}
                        />
                        {openTransIdx === i && (
                          <TransitionPicker
                            value={slide.transitionIn || 'none'}
                            onSelect={v => onUpdateSlide(i, { transitionIn: v })}
                    ...
                    <div
                      ...
                      style={{
                        borderColor: selectedIdx === i ? CLIP_COLORS[i % CLIP_COLORS.length] : 'transparent',
                        background: `${CLIP_COLORS[i % CLIP_COLORS.length]}28`,
                      }}>
                      {/* Thumbnail */}
                      {slide.src && (
                        <img src={slide.src} alt="" className="absolute inset-0 w-full h-full object-cover opacity-55 pointer-events-none"
                          style={{ filter: FILTERS.find(f => f.id === slide.filter)?.css || 'none' }} />
                      )}
                      <div className="absolute inset-0 pointer-events-none" style={{ background: `${CLIP_COLORS[i % CLIP_COLORS.length]}1a` }} />

                      {/* Text */}
                      <div className="relative z-10 h-full flex flex-col justify-between p-1.5 overflow-hidden">
                        <span className="text-[8px] font-bold text-white drop-shadow truncate">{slide.name || `Slide ${i+1}`}</span>
                        <span className="text-[8px] font-mono text-white/60">{slide.duration || 4}s</span>
                      </div>

                      {/* Delete on hover */}
                      <button onClick={e => { e.stopPropagation(); onDeleteSlide(i) }}
                        className="absolute top-0.5 right-0.5 w-4 h-4 rounded bg-red-600/80 text-white text-[8px] flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity z-20">✕</button>
                      ...
                      <div
                        ...
                        style={{ background: `${CLIP_COLORS[i % CLIP_COLORS.length]}88` }}
                        onMouseDown={e => {
                          e.preventDefault(); e.stopPropagation()
                          const startX = e.clientX, startDur = slide.duration || 4
                          const parentW = e.currentTarget.closest('[data-timeline]')?.getBoundingClientRect().width || 1
                          const secPerPx = totalDuration / parentW
                          const onMove = mv => onUpdateSlide(i, { duration: Math.max(1, Math.round((startDur + (mv.clientX - startX) * secPerPx) * 2) / 2) })
              ...
              {/* Playhead needle */}
              <div className="absolute top-0 h-full w-0.5 pointer-events-none z-30"
                style={{ left: `${progress * 100}%`, background: '#ec4899', boxShadow: '0 0 6px #ec4899' }}>
                <div className="absolute -top-1.5 -left-1.5 w-3 h-3 rotate-45" style={{ background: '#ec4899' }} />
              </div>
            </div>

            {/* Time ruler */}
            <div className="flex justify-between mt-1 px-0.5">
              {Array.from({ length: Math.min(10, Math.ceil(totalDuration) + 1) }).map((_, i) => {
                const t = (totalDuration / Math.max(1, Math.min(9, Math.ceil(totalDuration)))) * i
                return <span key={i} className="text-[8px] font-mono text-white/20">{fmt(t)}</span>
              })}
            </div>
          </>
        )}
...
// ─────────────────────────────────────────────────────────────────────────────
// SLIDE PROPERTIES PANEL (right sidebar)
// ─────────────────────────────────────────────────────────────────────────────
function SlideProperties({ slide, index, total, onUpdate, onDelete, onMoveLeft, onMoveRight,
                           globalDuration, globalFilter, globalAnimation, globalTransition,
                           setGlobalDuration, setGlobalFilter, setGlobalAnimation, setGlobalTransition, onApplyGlobal }) {
  if (!slide) return (
    <div className="flex-1 flex items-center justify-center text-white/20 text-sm p-4 text-center">
      <div><p className="text-3xl mb-2">🎞</p><p>Select a slide or add images from the left panel</p></div>
    </div>
  )
    ...
    <div className="flex-1 overflow-y-auto" style={{ scrollbarWidth: 'thin' }}>
      <div className="p-4 space-y-4">
        ...
        <div className="flex items-center justify-between">
          <p className="text-[10px] font-bold text-white/40 uppercase tracking-widest">Slide {index + 1} of {total}</p>
          <div className="flex gap-1">
            <button onClick={() => onMoveLeft(index)} disabled={index === 0}
              className="w-6 h-6 rounded text-white/30 hover:text-white hover:bg-white/10 text-xs disabled:opacity-20 transition-all">←</button>
            <button onClick={() => onMoveRight(index)} disabled={index === total - 1}
              className="w-6 h-6 rounded text-white/30 hover:text-white hover:bg-white/10 text-xs disabled:opacity-20 transition-all">→</button>
            <button onClick={() => onDelete(index)}
              className="w-6 h-6 rounded text-white/30 hover:text-red-400 hover:bg-red-950/30 text-xs transition-all">✕</button>
          </div>
        </div>

        {/* Thumbnail preview with filter */}
        <div className="aspect-video rounded-xl overflow-hidden border border-white/10" style={{ background: '#000' }}>
          {slide.src && <img src={slide.src} alt="" className="w-full h-full object-cover" style={{ filter: filterCss || 'none' }} />}
        </div>
        ...
        <div>
          <div className="flex justify-between text-[11px] text-white/60 mb-2">
            ...
            <span className="font-mono text-violet-300">{slide.duration || 4}s</span>
          </div>
          <input type="range" min={1} max={15} step={0.5} value={slide.duration || 4}
        ...
        <div>
          ...
          <div className="grid grid-cols-2 gap-1">
            {ANIMATIONS.map(a => (
              <button key={a.id} onClick={() => onUpdate(index, { animation: a.id })}
        ...
        {/* Transition In */}
        <div>
          <p className="text-[11px] font-semibold text-white/60 mb-2">Transition In</p>
          <div className="grid grid-cols-3 gap-1">
            {TRANSITIONS.map(t => (
              <button key={t.id} onClick={() => onUpdate(index, { transitionIn: t.id })}
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-1.5">
            {FILTERS.map(f => (
              <div key={f.id} onClick={() => onUpdate(index, { filter: f.id })}
                ...
                style={{ width: 46 }}>
                {slide.src
                  ? <img src={slide.src} alt="" className="w-full h-7 object-cover" style={{ filter: f.css || 'none' }} />
                  : <div className="w-full h-7" style={{ background: '#333', filter: f.css || 'none' }} />
                }
                <p className="text-[8px] text-center py-0.5 text-white/40 truncate px-0.5">{f.label}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="border-t border-white/8 pt-4 space-y-3">
          <p className="text-[10px] font-bold text-white/35 uppercase tracking-wider">Apply to All Slides</p>
          <div className="grid grid-cols-2 gap-2">
            <div>
              <label className="text-[9px] text-white/35 block mb-1">Duration</label>
              <select value={globalDuration} onChange={e => setGlobalDuration(Number(e.target.value))}
                className="w-full text-[10px] bg-white/5 border border-white/10 rounded-lg px-2 py-1 text-white">
                {[1,2,3,4,5,6,8,10,15].map(d=><option key={d} value={d}>{d}s</option>)}
              </select>
            </div>
            <div>
              <label className="text-[9px] text-white/35 block mb-1">Transition</label>
              <select value={globalTransition} onChange={e => setGlobalTransition(e.target.value)}
                className="w-full text-[10px] bg-white/5 border border-white/10 rounded-lg px-2 py-1 text-white">
                {TRANSITIONS.map(t=><option key={t.id} value={t.id}>{t.icon} {t.label}</option>)}
              </select>
            </div>
            <div>
              <label className="text-[9px] text-white/35 block mb-1">Motion</label>
              <select value={globalAnimation} onChange={e => setGlobalAnimation(e.target.value)}
                className="w-full text-[10px] bg-white/5 border border-white/10 rounded-lg px-2 py-1 text-white">
                {ANIMATIONS.map(a=><option key={a.id} value={a.id}>{a.label}</option>)}
              </select>
            </div>
            <div>
              <label className="text-[9px] text-white/35 block mb-1">Filter</label>
...
// ─────────────────────────────────────────────────────────────────────────────
// INTERCEPT AssetPanel image clicks → add as slides
// AssetPanel calls store.addShape({ type:'image', src, ... }) when an image is clicked.
// We patch addShape on the editorStore instance to redirect image clicks to slides.
// Audio clicks go straight to audioStore.setTrack() (unchanged — AssetPanel does this itself).
// ─────────────────────────────────────────────────────────────────────────────
  ...
  useEffect(() => {
    ...
    store.addShape = (shape) => {
      if (shape?.type === 'image' && shape?.src) {
        ...
        const name = shape.src.split('/').pop().split('?')[0].replace(/\.[^.]+$/, '') || 'Image'
        onImageAdded({ src: shape.src, name })
        // Do NOT call orig — no canvas shapes in images-to-video mode
      }
      // shapes other than image (should not happen here) are silently ignored
    }
    return () => { store.addShape = orig }
  }, [onImageAdded])
}
...
export default function ImagesToVideoPage() {
  ...
  // ── Global defaults for new slides ─────────────────────────────────────
  ...
  // ── Resolution ─────────────────────────────────────────────────────────
  const [resolution, setResolution] = useState(RESOLUTIONS[1])   // 1280×720
  ...
  // ── Push undo snapshot whenever slides change (except during undo/redo) ─
  useEffect(() => {
  ...
  // Autosave — debounced 3s after any slide/title/resolution change
  useEffect(() => {
  ...
  // ── Derived ────────────────────────────────────────────────────────────
  const totalDuration = slides.reduce((t, s) => t + (s.duration || 4), 0)
    + Math.max(0, slides.length - 1) * TRANS_DUR
  const sel = slides[selectedIdx] ?? null

  // ── Intercept AssetPanel image clicks ─────────────────────────────────
  const handleImageAdded = useCallback(({ src, name }) => {
    setSlides(prev => {
      const ns = [...prev, {
        id: uid(), src, name,
  ...
  // Reset audio when leaving this page
...
useEffect(() => {
  ...
  async function loadDesign() {
    try {
      ...
      if (!isVideoTemplate) {
        ...
        toast.error(
          'This is not an Images-to-Video design'
        )
  ...
  h: canvas.canvasSize.height || 720,
      ...
      // =====================================================
      // Restore slides safely
      // =====================================================
      let restoredSlides = []
      if (
        Array.isArray(canvas.pages) &&
      ...
      // =====================================================
      // Restore selected slide
      // =====================================================

      if (
        typeof canvas.currentPageIndex ===
        'number'
      ) {
  ...
  Math.max(
    0,
    Math.min(
      canvas.currentPageIndex || 0,
      restoredSlides.length - 1
    )
  )
)
      }
      ...
      // =====================================================
      // Restore audio (multi-track first, then legacy)
      // =====================================================
  ...
  function moveSlide(from, to) {
    ...
    setSlides(prev => { const a = [...prev]; const [x] = a.splice(from, 1); a.splice(to, 0, x); return a })
    setSelectedIdx(to)
  }
  function applyGlobal() {
    setSlides(prev => prev.map(s => ({
      ...s, duration: globalDuration, filter: globalFilter,
      animation: globalAnimation, transitionIn: globalTransition,
    })))
    toast.success('Applied to all slides')
  }

  // ── Shared save core (used by both manual save and autosave) ───────────
  async function doSave(currentTitle = title) {
    ...
    const canvas_json = {
      ...
      audioTracks: audioStore.tracks?.length > 0 ? audioStore.serialise() : null,
      audioTrack: null,
    }
    ...
    if (id) {
      await api.put(`/designs/${id}`, { title: currentTitle, canvas_json, thumbnail_url })
    } else {
      const res = await api.post('/designs', { title: currentTitle, canvas_json, thumbnail_url })
  ...
  async function commitTitleRename(newTitle) {
    ...
    if (id) {
      try { await api.put(`/designs/${id}`, { title: trimmed }) } catch { /* silent */ }
    }
  }
  ...
  useEffect(() => {
    ...
    const tick = () => {
      ...
      setPlayheadSec(cur)
      // Drive every audio track based on master playhead
  ...
  function togglePlay() {
    if (isPlaying) {
      ...
      // Stop any tracks that were being solo-previewed from the AudioPlayerBar
      audioStore.pauseAll()
      // Seek all audio tracks to correct positions for the current playhead
  ...
  function seekTo(sec) {
    ...
    // Reposition all audio tracks to match new playhead
...
useEffect(() => {
  ...
  function drawFrame() {
    ...
    for (let i = 0; i < slides.length; i++) {
      ...
      if (playheadSec < elapsed + transDur) {
        ...
        const img1 =
          loadedImagesRef.current[slides[i].src]

        const img2 =
          loadedImagesRef.current[slides[i + 1]?.src]
    ...
    const img =
      loadedImagesRef.current[slide.src]
  ...
  async function exportVideo() {
    if (!slides.length) { toast.error('Add at least one image first'); return }
    setExporting(true)
    try {
      const { w, h } = resolution; const FPS = 30
      setExportProgress('Loading images…')
      const imgs = await Promise.all(slides.map(s => new Promise(res => {
        const img = new Image(); img.crossOrigin = 'anonymous'
        img.onload = () => res(img); img.onerror = () => res(null); img.src = s.src
      })))
        ...
        try {
          ...
          for (const trackState of activeTracks) {
            ...
            try {
              ...
              console.warn('Export: failed to load track', trackState.track?.name, trackErr)
            }
          }

          setExportProgress(`Loaded ${exportAudioEls.length} audio track${exportAudioEls.length !== 1 ? 's' : ''}…`)
        } catch (e) {
      ...
      function syncExportAudio(masterTimeSec) {
        for (const { el, trackState } of exportAudioEls) {
          ...
          const clipDur = (trackState.trimEnd ?? el.duration ?? 0) - (trackState.trimStart ?? 0)
          const clipEnd = offset + clipDur
          const inWindow = masterTimeSec >= offset && masterTimeSec < clipEnd

          if (inWindow) {
            const targetTime = (trackState.trimStart ?? 0) + (masterTimeSec - offset)
      ...
      if (audioDest) audioDest.stream.getAudioTracks().forEach(t => combined.addTrack(t))

      const recorder = new MediaRecorder(combined, { mimeType: mime, videoBitsPerSecond: 16_000_000 })
      const chunks   = []
      recorder.ondataavailable = e => { if (e.data.size > 0) chunks.push(e.data) }

      // Total export duration = max(slides duration, last audio track end)
      const slideDur = slides.reduce((t, s) => t + (s.duration || 4), 0)
        + Math.max(0, slides.length - 1) * TRANS_DUR
      ...
      recorder.onstop = () => {
        exportAudioEls.forEach(({ el }) => { try { el.pause(); el.src = '' } catch {} })
        if (audioCtx) { try { audioCtx.close() } catch {} }
        const blob = new Blob(chunks, { type: mime })
        const url  = URL.createObjectURL(blob)
        Object.assign(document.createElement('a'), { href: url, download: 'images-to-video.webm' }).click()
        setTimeout(() => URL.revokeObjectURL(url), 10000)
        setExporting(false); setExportProgress('')
        const nTracks = exportAudioEls.length
        toast.success(`Exported ${Math.round(totalExportDur)}s video${nTracks > 0 ? ` · ${nTracks} audio track${nTracks !== 1 ? 's' : ''}` : ''}!`)
      }
      ...
      await new Promise(resolve => {
        let si = 0, se = 0, inT = false, te = 0, lastTs = null, rendered = 0
        ...
        function tick(ts) {
          ...
          if (inT && i2) {
            ...
            renderTransitionFrame(ctx, i1, i2, tp, slides[si + 1]?.transitionIn || 'fade', w, h)
            if (tp >= 1) {
              si++; inT = false; te = 0; se = 0
              if (si >= slides.length) { resolve(); return }
            }
          } else {
            se += dt
            ...
            if (se >= dur) {
              if (si + 1 < slides.length) { inT = true; te = 0; se = 0 }
              else { resolve(); return }
            }
          }

          if (Math.floor(rendered) !== Math.floor(rendered - dt)) {
            setExportProgress(`Rendering ${Math.round(rendered)} / ${Math.round(totalExportDur)}s…`)
          }
      ...
      console.error(err); toast.error('Export failed: ' + err.message)
      setExporting(false); setExportProgress('')
    }
  }
  ...
  // ─────────────────────────────────────────────────────────────────────────
  return (
    <div className="h-screen flex flex-col overflow-hidden text-white" style={{ background: '#0a0814' }}>
      <style>{`
      ...
      {/* ── Top nav ──────────────────────────────────────────────────────── */}
      <div className="h-12 border-b border-white/10 flex items-center px-3 gap-2 shrink-0"
        ...
        <span className="text-[10px] text-white/40 shrink-0">{slides.length} slide{slides.length !== 1 ? 's' : ''}</span>
        {totalDuration > 0 && <>
          <span className="text-[10px] text-white/30 shrink-0">·</span>
          <span className="text-[10px] text-white/40 shrink-0">{fmt(totalDuration)}</span>
        </>}
        ...
        <select value={`${resolution.w}x${resolution.h}`}
          onChange={e => { const r = RESOLUTIONS.find(r => `${r.w}x${r.h}` === e.target.value); if (r) setResolution(r) }}
          ...
          style={{ background: 'rgba(124,58,237,0.25)', border: '1px solid rgba(124,58,237,0.45)' }}>
          {RESOLUTIONS.map(r => <option key={r.label} value={`${r.w}x${r.h}`} style={{ background: '#1a0a2e' }}>{r.label}</option>)}
        </select>

        {/* Manual save */}
        <button onClick={saveDesign} disabled={saving || !slides.length}
        ...
        {/* Export */}
        <button onClick={exportVideo} disabled={exporting || !slides.length}
          className="text-xs px-3 py-1.5 rounded-lg font-semibold text-white hover:brightness-110 transition-all disabled:opacity-40 shrink-0"
          style={{ background: 'linear-gradient(90deg,#7c3aed,#ec4899)' }}>
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="flex-1 flex flex-col overflow-hidden min-w-0">
          ...
          <div className="flex-1 flex items-center justify-center overflow-hidden relative"
            style={{ background: 'radial-gradient(ellipse at center, #1a1230 0%, #0a0814 100%)' }}>

            {slides.length === 0 ? (
              <div className="text-center text-white/20 pointer-events-none select-none px-8">
                <p className="text-6xl mb-4">🎞</p>
                <p className="text-lg font-semibold">Click images in the left panel to add slides</p>
                <p className="text-sm mt-2 text-white/15">Use the Images tab to browse uploads or search online · Audio tab for background music</p>
              </div>
            ) : (
              <div className="relative rounded-xl overflow-hidden shadow-2xl border border-white/10"
                style={{ maxWidth: '85%', maxHeight: '85%', aspectRatio: `${resolution.w}/${resolution.h}`, width: '100%' }}>
            ...
            {exporting && (
              <div className="absolute inset-0 flex flex-col items-center justify-center"
                ...
                <p className="text-sm font-semibold text-white mb-1">Rendering video…</p>
                <p className="text-xs text-white/50 max-w-xs text-center">{exportProgress}</p>
                <p className="text-[10px] text-white/25 mt-3">Do not close this tab</p>
              </div>
            )}
          </div>
        ...
        <div className="w-72 shrink-0 border-l border-white/8 flex flex-col overflow-hidden"
