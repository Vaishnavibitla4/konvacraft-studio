...
export default function EditorPage() {
  ...
  const { canvasSize, loadDesign, selectedId, setSelected, pages, currentPageIndex, isPresentationMode } = store
  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        // Restore audio tracks if design had any saved
  ...
  // Stop audio when leaving editor
  ...
  const handleSave = useCallback(async (silent = false) => {
    ...
    try {
      // Always read latest state from store to avoid stale closure issues
      ...
      if (stage) {
        try {
          ...
          // Reset stage transform so coordinates match world space
          const prevX = stage.x(); const prevY = stage.y(); const prevScale = stage.scaleX()
          stage.position({ x: 0, y: 0 }); stage.scale({ x: 1, y: 1 })

          // Get Konva's canvas as base (shapes without videos)
          const baseDataURL = stage.toDataURL({
            x: 0, y: 0, width: cw, height: ch,
            pixelRatio: thumbRatio, mimeType: 'image/jpeg', quality: 0.8,
          })

          // Restore stage transform immediately
          stage.position({ x: prevX, y: prevY }); stage.scale({ x: prevScale, y: prevScale })

          // Composite video frames on top using an offscreen canvas
          const currentPageShapes = allPages[cIdx]?.shapes || []
          const videoShapes = currentPageShapes.filter(s => s.type === 'video')

          let dataURL = baseDataURL

          if (videoShapes.length > 0) {
            // Build a composite canvas: Konva base + video frames drawn on top
            ...
            // Draw each video element's current frame on top
          ...
          const thumbRes = await api.post('/assets/upload-dataurl', { dataURL, folder: 'thumbnails' })
      ...
      await api.put(`/designs/${id}`, {
        ...
        canvas_json: {
          ...
          audioTrack: null, // clear legacy field
        },
        thumbnail_url,
      })

      if (!silent) { setSaved(true); setTimeout(() => setSaved(false), 2500); toast.success('Design saved!') }
    } catch (err) {
  ...
  useEffect(() => { hasUnsavedRef.current = true }, [shapes, pages, title])

  // Auto-save every 30s
  useEffect(() => {
    const t = setInterval(() => {
      if (hasUnsavedRef.current) {
        handleSave(true).then(() => { hasUnsavedRef.current = false })
      }
    }, 30000)
  ...
  return (
    <div className="h-screen flex flex-col overflow-hidden" style={{ background: '#111118' }}>
      <style>{`
      ...
      <div className="editor-nav h-11 border-b border-white/10 flex items-center px-4 gap-3 shrink-0 z-30 shadow-xl">
        <button onClick={async () => {
            if (hasUnsavedRef.current) {
              try { await handleSave(true); hasUnsavedRef.current = false } catch {}
            }
            navigate('/dashboard')
          }}
        ...
        {isPresentation && (
          <span className="px-2 py-0.5 rounded-full text-[9px] font-bold text-violet-300 border border-violet-500/40 uppercase tracking-wider"
            style={{ background: 'rgba(124,58,237,0.15)' }}>
            Presentation · {pages.length} {pages.length === 1 ? 'slide' : 'slides'}
          </span>
        )}
        ...
        {saved && !saving && (
          <span className="text-xs text-emerald-400 font-medium flex items-center gap-1 ml-1 saved-badge">✓ Saved</span>
        )}

        <div className="flex-1" />

        <div className="hidden xl:flex items-center gap-3 text-[10px] text-white/25">
          {[['Ctrl+Z', 'Undo'], ['Ctrl+0', 'Fit'], ['Del', 'Delete'], ['V', 'Select']].map(([k, l]) => (
            <span key={k}><kbd className="bg-white/10 border border-white/15 rounded px-1 py-0.5 font-mono">{k}</kbd> {l}</span>
          ))}
        </div>

        {selectedId && (
          <button onClick={() => setSelected(null)} className="text-xs text-white/30 hover:text-white/60 transition-colors">Esc</button>
        )}
      </div>

      {/* Toolbar */}
      <Toolbar stageRef={stageRef} onSave={() => handleSave(false)} saving={saving} designId={id} designTitle={title} />
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="flex-1 flex flex-col min-w-0 min-h-0 overflow-hidden">
          <CanvasArea stageRef={stageRef} onDurationChange={setMasterDuration} />
          <AudioPlayerBar isPresentation={isPresentation} masterDuration={masterDuration} />
        </div>
        ...
        <div className="flex shrink-0 h-full">
          ...
          {isPresentation && (
            <div className="shrink-0 border-l border-white/10 overflow-hidden flex flex-col"
          ...
          {/* Properties panel */}
          {selectedId && (
            <div className="w-72 shrink-0 border-l border-white/10 overflow-hidden flex flex-col"
              style={{ background: 'rgba(15,12,28,0.97)', animation: 'fadeIn 0.2s ease both' }}>
              <PropertiesPanel />
            </div>
          )}
        </div>
