...
import { Stage, Layer, Rect, Circle, Ellipse, Text, Line,
         RegularPolygon, Star, Arrow, Image as KonvaImage } from 'react-konva'
import useImage from 'use-image'
import Konva from 'konva'
import api from '../lib/api'

/**
 * EmbedPage — pixel-perfect read-only embed of a KonvaCraft design.
 * Uses the exact same react-konva primitives and props as CanvasArea.
 * Loaded at /embed/:id — no auth required.
 */
export default function EmbedPage() {
  ...
  useEffect(() => {
    async function load() {
      try {
        let data
        try       { data = (await api.get(`/designs/${id}/public`)).data }
        catch     { data = (await api.get(`/designs/${id}`)).data }
  ...
  // Compute scale whenever container or design changes
  useEffect(() => {
...
const ch = design?.canvasSize?.height || 800
  ...
  if (error) return (
    <div style={S.center}>
      <div style={{ textAlign: 'center' }}>
        ...
        <p style={{ fontWeight: 700, color: '#111', marginBottom: 4 }}>Design not found</p>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh',
                  background: '#f3f4f6', overflow: 'hidden', fontFamily: 'sans-serif' }}>
        ...
        <div style={S.bar}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
            <button style={S.zoomBtn}
              onClick={() => { setScale(s => { const n=Math.min(s*1.2,4); setZoom(Math.round(n*100)); return n }) }}>+</button>
            <span style={{ fontSize: 11, color: 'rgba(255,255,255,0.4)', minWidth: 42,
                           textAlign: 'center', fontFamily: 'monospace' }}>{zoom}%</span>
            <button style={S.zoomBtn}
              onClick={() => { setScale(s => { const n=Math.max(s/1.2,0.05); setZoom(Math.round(n*100)); return n }) }}>−</button>
          </div>
          <span style={{ fontSize: 11, color: 'rgba(255,255,255,0.2)' }}>KonvaCraft</span>
        </div>
      )}
...
// ─── Per-shape renderer — mirrors CanvasArea exactly ─────────────────────────
function EmbedShape({ shape }) {
  ...
  // Shadow helper — only add shadow props when the shape actually has one
  const shadow = shape.shadowBlur || shape.shadowOffsetX || shape.shadowOffsetY
    ? {
  ...
  switch (shape.type) {
    case 'rect':
      return <Rect {...base} width={shape.width ?? 100} height={shape.height ?? 100} />
...
// ─── Image shape — mirrors CanvasArea's ImageShape with filters ───────────────
...
// ─── Video shape — shows first frame, no playback in embed ───────────────────
...
function Spinner() {
  return (
    <div style={S.center}>
      <div style={{ textAlign: 'center' }}>
        <div style={S.logo}>K</div>
        <div style={{ width: 24, height: 24, border: '2px solid #7c3aed',
